...
class SoundPoolTool(
    ...
    init {
        SoundPool.Builder()
            ...
            .setAudioAttributes(
                ...
                /**
                 * 设置资源加载监听
                 * @param sp        – 来自 load() 方法的 SoundPool 对象
    ...
    private fun SoundPool.initIds() {
        soundIds.forEach { id ->
            soundMap["$id"] = load(App.context, id, 1)
        }
        if (isPlayDefaultSound == true) {
            getSystemDefaultRingtoneUri().also { uri ->
                when {
                    defaultSoundId != null -> {
                        soundMap[defaultName] = load(App.context, defaultSoundId, 1)
                    }
                    uri != null -> {
                        soundMap[defaultName] =
                            load(uriToPath(uri), 1)
                    }
                }
            }
        }
